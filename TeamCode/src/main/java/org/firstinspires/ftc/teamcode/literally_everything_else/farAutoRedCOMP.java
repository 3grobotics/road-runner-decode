...
@Disabled
@Autonomous(name = "farAutoRedCOMP", group = "competition")
public class farAutoRedCOMP extends LinearOpMode {
    ...
    private String orderToStringPre(java.util.List<SidePre> seq) {
        ...
        for (SidePre s : seq)
            sb.append(s == SidePre.LEFT ? "L" : (s == SidePre.RIGHT ? "R" : "M")).append(' ');
    ...
    private String orderToStringSpi1(java.util.List<SideSpi1> seq) {
        StringBuilder sb = new StringBuilder();
        for (SideSpi1 s : seq)
            sb.append(s == SideSpi1.LEFT ? "L" : (s == SideSpi1.RIGHT ? "R" : "M")).append(' ');
        return sb.toString().trim();
    }
    ...
    private String orderToStringSpi2(java.util.List<SideSpi2> seq) {
        StringBuilder sb = new StringBuilder();
        for (SideSpi2 s : seq)
            sb.append(s == SideSpi2.LEFT ? "L" : (s == SideSpi2.RIGHT ? "R" : "M")).append(' ');
        return sb.toString().trim();
    }
    ...
    public class Robot {
        ...
        public Action posservo2() {
            return new posservo2();
        }
        ...
        public Action posservo3() {
            return new posservo3();
        }
        ...
        public Action intake_stop() {
            return new intake_stop();
        }
        ...
        private class init implements Action {
            @Override
            public boolean run(@NonNull TelemetryPacket p) {
                flipper.setPosition(.21);
                tiltLeft.setPosition(0.420);
                tiltMid.setPosition(0.290);
                tiltRight.setPosition(0.310);
                pullLeft.setPower(.2);
                pullMid.setPower(.2);
        ...
        public Action shootByOrderPreload() {
            return new shootByOrderPreload();
        }
        ...
        public Action shootByOrderSpike1() {
            return new shootByOrderSpike1();
        }
        ...
        public Action shootByOrderSpike2() {
            return new shootByOrderSpike2();
        }

        // ── NEW: Non-spinning AprilTag aim (pipeline 0) with acquire + scan + offset.
        // If no tag shows up, it will gently scan for up to ACQUIRE_TIMEOUT_MS and then finish.
        private class aimRedTag1 implements Action {
            private final MecanumDrive drive1;
            ...
            private static final double AIM_OFFSET_DEG = -5.0;   // positive = aim that many deg to the RIGHT of tag
            private static final long LOCK_HOLD_MS = 150;

            // New: acquisition helpers
            private static final int PIPELINE_SETTLE_FRAMES = 10;   // wait a few frames after switching
            private static final long ACQUIRE_TIMEOUT_MS = 900; // max time we'll try to find a tag
            private static final long SCAN_SWEEP_MS = 280; // when to flip scan direction
            private static final double SCAN_YAW = 0.18; // gentle scanning yaw

            private int framesSinceSwitch = 0;
            private long acquireMs = 0L;
            private long scanFlipMs = 0L;
            private int scanDir = 1;

            aimRedTag1(MecanumDrive d1) {
                this.drive1 = d1;
            }
            ...
            public boolean run(@NonNull TelemetryPacket p) {
                if (!inited) {
                    if (limelight != null)
                        limelight.pipelineSwitch(0); // make sure we're on pipeline 0
                ...
                lastNs = now;
                acquireMs += (long) (dt * 1000.0);
                scanFlipMs += (long) (dt * 1000.0);

                // Give pipeline a few frames to settle before deciding we "don't see" a tag.
                if (framesSinceSwitch < PIPELINE_SETTLE_FRAMES) {
                    framesSinceSwitch++;
                    // gentle hold
                    drive1.setDrivePowers(new PoseVelocity2d(new Vector2d(0.0, 0.0), 0.0));
                    p.put("aim_state", "settling");
                    return true;
                }

                if (!haveTag) {
                    // No tag yet — perform a gentle scanning yaw for up to ACQUIRE_TIMEOUT_MS
                    if (scanFlipMs >= SCAN_SWEEP_MS) {
                        scanDir = -scanDir;
                        scanFlipMs = 0L;
                    }
                    double yaw = SCAN_YAW * scanDir;
                    yaw = Math.max(-AIM_YAW_MAX, Math.min(AIM_YAW_MAX, yaw));
                    drive1.setDrivePowers(new PoseVelocity2d(new Vector2d(0.0, 0.0), yaw));

                    p.put("aim_state", "scanning");
                    p.put("aim_scanDir", scanDir);
                    p.put("aim_acquire_ms", acquireMs);

                    // stop scanning after timeout so auto doesn't stall forever
                    if (acquireMs >= ACQUIRE_TIMEOUT_MS) {
                        drive1.setDrivePowers(new PoseVelocity2d(new Vector2d(0.0, 0.0), 0.0));
                        p.put("aim", "timeout — no tag");
                        return false; // finish gracefully, no tag found
                    }
                    return true; // keep scanning
                }

                // Tag acquired — PD hold with offset (note the subtraction so OFFSET shifts the target)
                ...
                p.put("aim_state", "tracking");
                p.put("aim_raw_tx", txDeg);
                p.put("aim_offset_deg", AIM_OFFSET_DEG);
                p.put("aim_tx_err", err);
        ...
        public Action aimRedTag1(MecanumDrive d1) {
            return new aimRedTag1(d1);
        }

        private class aimRedTag2 implements Action {
            private final MecanumDrive drive2;
            private boolean inited = false;
            private long lastNs = 0L;
            private long holdMs = 0L;
            private double prevErrDeg = 0.0;
            ...
            public boolean run(@NonNull TelemetryPacket p) {
                ...
                if (framesSinceSwitch < PIPELINE_SETTLE_FRAMES) {
                    framesSinceSwitch++;
                    // gentle hold
                    drive2.setDrivePowers(new PoseVelocity2d(new Vector2d(0.0, 0.0), 0.0));
                    p.put("aim_state", "settling");
                    return true;
                }
                ...
                if (!haveTag) {
                    ...
                    double yaw = SCAN_YAW * scanDir;
                    yaw = Math.max(-AIM_YAW_MAX, Math.min(AIM_YAW_MAX, yaw));
                    drive2.setDrivePowers(new PoseVelocity2d(new Vector2d(0.0, 0.0), yaw));
        ...
        public Action aimRedTag2(MecanumDrive d2) {
            return new aimRedTag2(d2);
        }



        private class aimRedTag3 implements Action {
            private final MecanumDrive drive3;
            private boolean inited = false;
            private long lastNs = 0L;
            private long holdMs = 0L;
            private double prevErrDeg = 0.0;
            ...
            aimRedTag3(MecanumDrive d3) {this.drive3 = d3;}

            @Override
            public boolean run(@NonNull TelemetryPacket p) {
                if (!inited) {
                    if (limelight != null) limelight.pipelineSwitch(0); // make sure we're on pipeline 0
                    lastNs = System.nanoTime();
                    holdMs = 0L;
                    prevErrDeg = 0.0;
                    framesSinceSwitch = 0;
                ...
                if (framesSinceSwitch < PIPELINE_SETTLE_FRAMES) {
                    framesSinceSwitch++;
                    // gentle hold
                    drive3.setDrivePowers(new PoseVelocity2d(new Vector2d(0.0, 0.0), 0.0));
                    p.put("aim_state", "settling");
                    return true;
                }
                ...
                if (!haveTag) {
                    ...
                    double yaw = SCAN_YAW * scanDir;
                    yaw = Math.max(-AIM_YAW_MAX, Math.min(AIM_YAW_MAX, yaw));
                    drive3.setDrivePowers(new PoseVelocity2d(new Vector2d(0.0, 0.0), yaw));
        ...
        public Action aimRedTag3(MecanumDrive d3) {return new aimRedTag3(d3);}
    }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        /* ---- traj / action setup ---- */
        Pose2d initialPose = new Pose2d(61, 8.5, Math.toRadians(0));
        Pose2d endOfPreload = new Pose2d(50, 14, Math.toRadians(340));
        Pose2d endOfgospike1 = new Pose2d(40, 47, Math.toRadians(180));
        Pose2d endOfscoreSpike1 = new Pose2d(50, 14, Math.toRadians(340));
        Pose2d endOfgospike2 = new Pose2d(20, 47, Math.toRadians(180));
        Pose2d endOfscoreSpike2 = new Pose2d(50, 14, Math.toRadians(340));
        Pose2d endOfgospike3 = new Pose2d(-8, 47, Math.toRadians(180));
        Pose2d endOfscoreSpike3 = new Pose2d(50, 14, Math.toRadians(340));

        MecanumDrive drive = new MecanumDrive(hardwareMap, initialPose);
        MecanumDrive drive1 = new MecanumDrive(hardwareMap, initialPose);
        MecanumDrive drive2 = new MecanumDrive(hardwareMap, initialPose);
        MecanumDrive drive3 = new MecanumDrive(hardwareMap, initialPose);
...
//.afterDisp(0, robot.thing3())
        Action preload = drive.actionBuilder(initialPose)
                ...
                .stopAndAdd(robot.aimRedTag1(drive1))  // use unified aim with acquire/scan/offset
                .waitSeconds(.4)
                .stopAndAdd(robot.shootByOrderPreload())
                .build();

        Action gospike1 = drive.actionBuilder(endOfPreload)
                .afterDisp(0, robot.posservo3())
                .setTangent(Math.toRadians(90))
                .splineToLinearHeading(new Pose2d(60, 47, Math.toRadians(180)), Math.toRadians(90))
                .afterDisp(0, robot.intake())
                .strafeToLinearHeading(new Vector2d(40, 47), Math.toRadians(180))
                .build();

        Action scoreSpike1 = drive.actionBuilder(endOfgospike1)
                .afterDisp(0, robot.init())
                .strafeToLinearHeading(new Vector2d(50, 14), Math.toRadians(340))
                .afterDisp(0, robot.intake_stop())
                .stopAndAdd(robot.aimRedTag2(drive2))  // unified aim again
                .waitSeconds(.4)
                .stopAndAdd(robot.shootByOrderSpike1())
                .build();

        Action gospike2 = drive.actionBuilder(endOfscoreSpike1)
                .afterDisp(0, robot.posservo3())
                .setTangent(Math.toRadians(90))
                .splineToLinearHeading(new Pose2d(40, 47, Math.toRadians(180)), Math.toRadians(180))
                .stopAndAdd(robot.intake())
                .strafeToLinearHeading(new Vector2d(20, 47), Math.toRadians(180))
                .build();

        Action scoreSpike2 = drive.actionBuilder(endOfgospike2)
                .afterDisp(0, robot.init())
                .afterDisp(0, robot.intake_stop())
                .setTangent(Math.toRadians(0))
                .splineToSplineHeading(new Pose2d(50, 47, Math.toRadians(0)), Math.toRadians(0))
                .setTangent(Math.toRadians(0))
                .splineToSplineHeading(new Pose2d(50, 14, Math.toRadians(340)), Math.toRadians(270))
                .stopAndAdd(robot.aimRedTag3(drive3))  // and a third time
                .waitSeconds(.5)
                .stopAndAdd(robot.shootByOrderPreload())
                .build();

        /*Action gospike3 = drive.actionBuilder(endOfPreload)
                .afterDisp(0, robot.posservo3())
        ...
        Action scoreSpike3 = drive.actionBuilder(endOfgospike3)
                .afterDisp(0, robot.intake_stop())
                .afterDisp(0, robot.init())
        ...
        Action offLine = drive.actionBuilder(endOfscoreSpike2)
                .setTangent(Math.toRadians(180))

...
@Disabled
@Config
@TeleOp(name = "teleop Red no turret", group = "TeleOp")
/* opmode*/
public class teleopRedNoturret extends LinearOpMode {
    ...
    // [CHANGE 1/3] Added accumulator variable
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            // [CHANGE 2/3] REMOVED "saw24 = false" here.
            // It is now reset inside the "majority of ll stuff" block below.
            // This ensures the turret logic uses the value from the PREVIOUS loop cycle.
            ...
            // [CHANGE 3/3] UPDATED TURRET LOGIC (ACCUMULATOR)
            // -------------------------------------------------------------------------
            ...
            // 5. Base Servo Position
            double baseServoDegrees = relativeAngle + 151.5;

            // 6. VISION INTEGRATOR
            // If we saw tag 24 in the previous loop cycle, we adjust the offset.
            if (saw24) {
                // Low speed = smooth (0.04). High speed = fast (0.1).
                double TRACKING_SPEED = 0.2;

                // Flip this to 1.0 if turret moves AWAY from target.
                double DIRECTION_FLIP = -1.0;

                // Accumulate Error
                visionOffsetDeg += (aimErrorDeg * TRACKING_SPEED * DIRECTION_FLIP);

                // Optional: Clamp offset to reasonable limits (e.g., +/- 90 deg)
                visionOffsetDeg = Range.clip(visionOffsetDeg, -25, 25);
            }

            double finalServoDegrees = baseServoDegrees + visionOffsetDeg + AIM_OFFSET_DEG_LOCAL;

            // 7. Clamp safety for the 0-303 range
            if (finalServoDegrees < 0) finalServoDegrees = 0;
            if (finalServoDegrees > 303) finalServoDegrees = 303;
            ...
            /* majority of ll stuff */ {
                // Reset saw24 HERE so it is fresh for the tag loop below,
                // but persists for the NEXT loop's turret logic.
                saw24 = false;
            ...
            */
            ...
            /* telemetry            */ {
                ...
                telemetry.addData("Vision Offset", "%.2f", visionOffsetDeg); // Vision Telemetry
